---
title: Issue Codes - DronaHQ Dashboard
...
DronaHQ Dashboard and provides additional context.
...
issue within DronaHQ Dashboard, but instead a problem with the underlying
...
```
DronaHQ Dashboard encountered an error while running a command.
```

Something unexpected happened, and DronaHQ Dashboard encountered an error while
running a command. Please reach out to your administrator.

## Issue 1011

```
DronaHQ Dashboard encountered an unexpected error.
```

Something unexpected happened in the DronaHQ Dashboard backend. Please reach out
...
Your instance of DronaHQ Dashboard doesn't have a results backend configured, which is needed for asynchronous queries. Please contact an administrator for further assistance.
...
The submitted query might be too complex to run under the time limit defined by your DronaHQ Dashboard administrator. Please double check your query and verify if it can be optimized. Alternatively, contact your administrator to increase the timeout period.

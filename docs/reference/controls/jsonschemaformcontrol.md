...
The JSON Schema Form (JSF) control is a composite form control that generates dynamic forms based on JSON schemas. It does not maintain state and utilizes React components to render and manage forms efficiently. This control is useful for applications where dynamic form generation is required based on predefined JSON structures.
...
The JSON Schema Form control supports both static and dynamic data binding. Static data binding involves pre-defined JSON structures, while dynamic data binding enables form updates based on real-time inputs or external data sources.
...
### UI Schema
The UI Schema defines additional functionalities such as placeholders, focus behaviors, and input field modifications. It enhances user experience by specifying how fields should be displayed and interacted with.
...
### Form Data
The Form Data represents the submitted values in JSON format. It holds the final output once the user interacts with and completes the form.
...
The CDNs can be provided if needed:  
...
| Events      | Supports `Click` as the only event trigger. |  
...
| Event  | Description |
|--------|-------------|
| Click | Triggers when the form is submitted or interacted with. |
...
| `{{jsonschemaformcontrol.json_schema.PROPERTIES_NAMES}}` | Lists all properties defined in the JSON schema. |
| `{{jsonschemaformcontrol.ui_schema.PROPERTIES_NAMES}}` | Lists all properties defined in the UI schema, including display settings. |

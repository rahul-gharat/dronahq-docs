...
### Upadting Partial-Multiple Rows using Google Sheets

Let's see  for scenarios where some fields in your dataset may need to be left blank or filled with null. We will cover the entire setup, including using JavaScript to manipulate the data and update the Google Sheets rows effectively.


Suppose our database looks somethings like this from the google sheet:
...
Now in the action flow, we need to add a JS Code action block, for a simple trasformation of the edited data from a tablegrid control. You can check out the [Building Interactive table](https://docs.dronahq.com/building-apps-guides/building-interactive-tables/#editable-table-columns) article to know about editable columns in table grid control.

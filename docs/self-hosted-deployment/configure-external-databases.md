...
```shell
sudo docker compose exec -T mongodb sh -c 'mongorestore --host=localhost -u <%user%> -p <%password%> --db <%database_name%> --authenticationDatabase admin --archive' < mongo-init.dump
```
